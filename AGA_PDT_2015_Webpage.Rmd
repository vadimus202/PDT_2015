...
```{r logo, fig.width=0.75*309/72, fig.height=0.75*163/72}
img <- readPNG('aga_logo.png')
grid.raster(img)
```

```{r nash, fig.width=0.50*757/72, fig.height=0.50*257/72}
img <- readJPEG('nashville_guitar.jpg')
grid.raster(img)
```
...
datatable(
  ...
  options=list(pageLength = 10, lengthMenu = c(5, 10, 20, 50, 100, 500)),
  rownames=FALSE)
...
Bar <- gvisBarChart(dat.city, options = list(
  series = "[{color: 'red'}]",
  titleTextStyle="{fontSize:20}",
  width=900,height=500, legend='none', 
  title="Top Cities"))
plot(Bar)
...
# Data Source
Official attendee list can be downloaded from [here](https://www.agacgfm.org/PDT2015/Home.aspx)
